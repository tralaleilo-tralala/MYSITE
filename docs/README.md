...
## Текстовые информаторы	
1. JSON (данный формат легковесный, легко читаемый.Структура ключ - значения. )
({"name": "John", "age": 30}) Исспользуется в RestApi
2. XML -структурированный формат с поддержкой схем xsd
<user>
	<name>John</name>
	<age>0</age>
</user>

3. YML - упрощенный синтаксис, удобен для конфигурации.
user:
	name: John
	age: 30
Используется в Docker и Cybernatiс.

## Выбор протокола и форматы.
Для WebApi - HTTP, HTTPS + JSON
Для стриминга, чатов и социальных сетей - Web Socket + Message pack
